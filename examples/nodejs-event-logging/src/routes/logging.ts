...
import { DescribeEventListenerResponse } from '@steadybit/event-kit-api';

export const router = express.Router();

router.get('/events/logging', (_, res) => {
	const response: DescribeEventListenerResponse = {
		id: 'com.github.steadybit.example.event.logging',
		label: 'Event Logging Listener',
		...
		description: 'Prints the received event payload to the console to illustrate the event API.',
		version: String(Date.now()),
		includeEnvironmentVariables: [],
		listenTo: ['*'],
		listen: {
			method: 'post',
			path: '/events/logging/listen'
		}
	};
	res.json(response);
});

router.post('/events/logging/listen', (req, res) => {
