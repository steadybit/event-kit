...
func TestEventListenerRequestBody(t *testing.T) {
	v := EventListenerRequestBody{
		Environment: Ptr(Environment{
			Id:   "test",
			Name: "gateway",
		}),
		EventName: "experiemntStarted",
		EventTime: time.Now(),
...
func TestActionList(t *testing.T) {
	v := EventListenerList{
		EventListeners: []DescribingEndpointReference{
			{
				Get,
				"/events/mylisteners",
			},
...
func TestEventListenerDescription(t *testing.T) {
	v := EventListenerDescription{
		...
		Id:                          "com.steadybit.example.events.kubernetes.rollout-restart",
		IncludeEnvironmentVariables: Ptr([]string{"ENV1", "ENV2"}),
		Label:                       "Rollout Restart Deployment",
		Listen: MutatingEndpointReference{
			Post,
			"/events/mylistener/listen",
		},
		ListenTo: Ptr([]string{"experimentStarted"}),
		Version:  "1.1.0",
	}
	markAsUsed(t, v)
}
